...
my $startDiff_StopingInt = 340; # seconds from the start of the timeframe to decide to stop
my $endDiff_StopingInt = 370; # seconds from the start of the timeframe to decide to stop
my $startDiff_DecisionInt = 380; # seconds from the start of the timeframe to decide to play
my $endDiff_DecisionInt = 440; # 0 seconds from the start of the timeframe to decide to play

my $crt_blocks_threshold = 6; #threshold for number of blocks from start of timeframe
my $first_blocks_threshold = 5; #threshold for number of blocks from start of timeframe
my $second_blocks_threshold = 4; #threshold for number of blocks from start of timeframe
my $third_blocks_threshold = 5; #threshold for number of blocks from start of timeframe
...
my $filename = 'control_order_previous_log.txt';
...
while (1) 
{
	...
	my $startDiff_Stoping = $crtTime - $startTime;
	my $endDiff_Stoping = $endTime - $crtTime;
	
	#in the middle of the crtTF we request speed and price decrease in case we mined this TF
	# there is a delay between the request and the actual decrease of the speed
	
	if (($startDiff_Stoping > $startDiff_StopingInt )&& ( $startDiff_Stoping < $endDiff_StopingInt))
	{
	  #do something
	  print "stoping at time  $startDiff_Stoping compare to $startDiff_StopingInt and $endDiff_StopingInt \n";
	...
	# at the end of the TF we decide what to do at the next TF
	if ( ($startDiff_Stoping > $startDiff_DecisionInt ) && ( $startDiff_Stoping < $endDiff_DecisionInt))
	{
	  #do something
	  print "in interval \n";
	  
	  # if crt is high, we assume the next TF is lower
	  if ( $crtBlocks{'noOfBlocks'} > $crt_blocks_threshold )
	  {
		print "crt TF higher then $crt_blocks_threshold \n";
		$currentHighSpeed = 0;	  
	  }
	  else
	  {
		# the crt is low.We should check the previous
		if ( $lastXBlocks[0]{'noOfBlocks'} > $first_blocks_threshold )
		{
			print " TF  $lastXBlocks[0]{'timeFrame'} higher then $first_blocks_threshold \n";		
			$currentHighSpeed = 0;	  
		}
		else
		{
			if ( $lastXBlocks[1]{'noOfBlocks'} > $second_blocks_threshold )
			{
			print " TF  $lastXBlocks[1]{'timeFrame'} higher then $second_blocks_threshold \n";					
			$currentHighSpeed = 0;	  
			}
			else
			{
