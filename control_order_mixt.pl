...
my $fh_decrease;
my $decreaseFilename = "control_order_mixt_decrease_success.txt";
...
my $filename = 'control_order_mixt_log.txt';
...
while (1) 
{
	...
	# watchdog
	my $filename_wdg = 'wdg_control_order_mixt.txt';
	...
	# last start
	my $filename_start = 'start_control_order_mixt.txt';	
	...
	getMinPrice();
	 ...
	 if ( $currentHighSpeed == 1 )
	 {
		...
		{
			print "delay start mining $delayCurrentHighSpeed ! \n";
			$delayCurrentHighSpeed--;
		}
		else
		{
			print "MINING ! \n";
