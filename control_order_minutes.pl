...
while (1) 
{
	...
	##alina necompilat
      	...
      	my $endTime =   Time::Piece->strptime($while_tstmp,'%a %b %d %H:%M:%S %Y');
      	
      	my $startM= sprintf("%02s",$m*10);
      	my $startTime = $endTime->strftime("%a %b %d %H:$startM:%S %Y");
      	$startTime = Time::Piece->strptime($startTime,'%a %b %d %H:%M:%S %Y');
